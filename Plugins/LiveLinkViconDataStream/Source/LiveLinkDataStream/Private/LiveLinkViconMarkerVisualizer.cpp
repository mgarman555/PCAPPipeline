  ...
  : Super(ObjectInitializer)
{
  ...
  DebugMeshMaterial = ConstructorHelpers::FObjectFinder<UMaterial>(
                        TEXT("Material'/LiveLinkViconDataStream/Materials/DebugMeshMaterialFakeLight.DebugMeshMaterialFakeLight'"))
                        .Object;
  ...
  UnitSphereMesh = ConstructorHelpers::FObjectFinder<UStaticMesh>(
                     TEXT("StaticMesh'/LiveLinkViconDataStream/Meshes/S_1_Unit_Sphere.S_1_Unit_Sphere'"))
                     .Object;
...
TArray<FVector> ULiveLinkViconMarkerVisualizer::GetMarkerData() const
{
  ...
  SubjectFrameData.FrameData = FLiveLinkFrameDataStruct(FLiveLinkLocatorFrameData::StaticStruct());
  SubjectFrameData.StaticData = FLiveLinkStaticDataStruct(FLiveLinkLocatorStaticData::StaticStruct());

  // if the data has locator role we retre
  if (LiveLinkClient->DoesSubjectSupportsRole_AnyThread(SubjectName, ULiveLinkLocatorRole::StaticClass()))
  {
    if (LiveLinkClient->EvaluateFrame_AnyThread(SubjectName, ULiveLinkLocatorRole::StaticClass(), SubjectFrameData))
    {
      FLiveLinkLocatorFrameData& rMarkerFrameData = *SubjectFrameData.FrameData.Cast<FLiveLinkLocatorFrameData>();
      return rMarkerFrameData.Locators;
    }
  }
  else
  {
