...
EResult ViconStream::Connect(const FString& i_rServer, bool i_bRetimed, bool i_bLogOutput)
{
  ...
  UE_LOG(LogViconStream, Display, TEXT("Using client version %d.%d.%d.%d"), Output_GetVersion.Major, Output_GetVersion.Minor, Output_GetVersion.Point, Output_GetVersion.Revision);
...
EResult ViconStream::GetLabeledMarkers(TArray<FVector>& o_rMarkerList)
{
  // not available in retimed data
  if (!m_Client.IsMarkerDataEnabled().Enabled)
  {
...
EResult ViconStream::GetUnlabeledMarkers(TArray<FVector>& o_rMarkerList)
{
  // not available in retimed data
  if (!m_Client.IsUnlabeledMarkerDataEnabled().Enabled)
  {
  ...
  const std::string& InName, const TArray<std::string>& BoneNames, const TArray<std::string>& MarkerNames, FLiveLinkFrameDataStruct& OutSubject)
{
  ...
  if (SegmentCount.SegmentCount == 1)
  {
    ...
    {
      UE_LOG(LogViconStream, Log, TEXT("Failed to get Segment for %hs:%hs"), InName.c_str(), BoneNames[0].c_str());
      return false;
    }
    if (IsViconServerYup())
    {
  ...
  {
    UE_LOG(LogViconStream, Warning, TEXT("Vicon segments has %d segments while Livelink skeleton has %d bones"), SegmentCount.SegmentCount, BoneCount);
  }
  ...
  for (unsigned int j = 0; j < Available; ++j)
  {
    ...
    {
      UE_LOG(LogViconStream, Log, TEXT("Failed to get Segment for %hs:%hs"), InName.c_str(), BoneNames[j].c_str());
      return false;
    }
