...
ViconStreamProperties ViconStreamProperties::FromString(const FString& i_rPropsString)
{
  ...
  // NSDServerName
  FString NSDServerName;
  if (!FParse::Value(*i_rPropsString, TEXT("NSDServerName="), NSDServerName))
  {
    NSDServerName = "";
  }
  if (!NSDServerName.IsEmpty())
  {
    // NSDServerDomain
    FString NSDServerDomain;
    if (!FParse::Value(*i_rPropsString, TEXT("NSDServerDomain="), NSDServerDomain))
    {
      NSDServerDomain = "";
    }
    // NSDServerType
    FString NSDServerType;
    if (!FParse::Value(*i_rPropsString, TEXT("NSDServerType="), NSDServerType))
    {
      NSDServerType = "";
    }
    Props.m_NSDProperties = FNSDService(NSDServerName, NSDServerType, NSDServerDomain);
  }
  // SubjectFilter
  ...
  // Retimed: FParse doesn't handle bool
  FString Retimed;
...
const FString ViconStreamProperties::ToString() const
{
  ...
  if (m_NSDProperties.IsSet())
  {
    PropertiesString.Append(FString::Printf(TEXT("NSDServerName=\"%s\""), *m_NSDProperties.GetValue().m_Name));
    PropertiesString.Append(FString::Printf(TEXT("NSDServerType=\"%s\""), *m_NSDProperties.GetValue().m_Type));
    PropertiesString.Append(FString::Printf(TEXT("NSDServerDomain=\"%s\""), *m_NSDProperties.GetValue().m_Domain));
  }
  ...
  PropertiesString.Append(FString::Printf(TEXT("RetimeOffset=\"%f\""), m_RetimeOffset));
...
{
  // If NSD server name provided, resolve it to address before connecting
  FString ServerAddress;
  EResult ConnectionResult = EError;
  if (m_ViconStreamProps.m_NSDProperties.IsSet())
  {
    TOptional<TTuple<FString, uint32_t>> ResolvedServerAddress = ResolveServerAddress(m_ViconStreamProps.m_NSDProperties.GetValue());
    if (ResolvedServerAddress.IsSet())
    {
      auto ServerAddressPair = ResolvedServerAddress.GetValue();

      m_ViconStreamProps.m_ServerName = FText::FromString(ServerAddressPair.Get<0>());
      m_ViconStreamProps.m_PortNumber = ServerAddressPair.Get<1>();
    }
  }

  ServerAddress = ConstructServerAddress();
  UE_LOG(LogViconStream, Log, TEXT("Connecting to datastream on %s"), *ServerAddress);
  ConnectionResult = m_DataStream.Connect(ServerAddress, m_ViconStreamProps.m_bRetimed, m_ViconStreamProps.m_bLogOutput);

  if (ConnectionResult != ESuccess)
  {
...
{
  // Markers are in the format [ x1, y1, z1 ... xn, yn, zn ]
...
TArray<FName> FViconStreamFrameReader::GetGenericMarkerLocatorNames(unsigned int MarkerCount)
{
  TArray<FName> PropertyNames;
  for (unsigned int MarkerIndex = 0; MarkerIndex < MarkerCount; ++MarkerIndex)
  {
    PropertyNames.Emplace(FString::FromInt(MarkerIndex));
  }
  return PropertyNames;
}
...
void FViconStreamFrameReader::HandleMarkerData(bool bLabeled)
{
  ...
  // If subject is not present, we need to add it.
  if (!rCachedMarker.SubjectPresent)
  {
    rCachedMarker.SubjectPresent = true;
    // Marker data is now using locater role, which allows unlabeled marker count to vary between frames.
    // Components currently (5.6) visualize locater role are Live Link Data Preview and Vicon Marker Visualizer
    FLiveLinkStaticDataStruct StaticDataStruct(FLiveLinkLocatorStaticData::StaticStruct());
    FLiveLinkLocatorStaticData& rMarkerStaticData = *StaticDataStruct.Cast<FLiveLinkLocatorStaticData>();
    // Property names are generated from marker indices as markers are only named when associated with a Vicon subject
    if (bLabeled)
    {
      rMarkerStaticData.LocatorNames = GetGenericMarkerLocatorNames(MarkerCount);
    }
    rMarkerStaticData.bUnlabelledData = !bLabeled;
    m_pLiveLinkClient->PushSubjectStaticData_AnyThread(SubjectKey, ULiveLinkLocatorRole::StaticClass(), MoveTemp(StaticDataStruct));
  }

  // Frame Data
  FLiveLinkFrameDataStruct FrameDataStruct(FLiveLinkLocatorFrameData::StaticStruct());
  FLiveLinkLocatorFrameData& rMarkerFrameData = *FrameDataStruct.Cast<FLiveLinkLocatorFrameData>();
  // Get property values
  TArray<FVector>& rLocatorValues = rMarkerFrameData.Locators;
  ...
  const auto TranslationResult = bLabeled ?
                                   m_DataStream.GetLabeledMarkers(rLocatorValues) :
                                   m_DataStream.GetUnlabeledMarkers(rLocatorValues);
