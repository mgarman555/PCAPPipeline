...
private:
    ...
    // A "+" that opens a small popup window at the button (not a dropdown list, not inline).
    // The popup holds one field per hint; OnCreate gets the trimmed values on commit.
    TSharedRef<SWidget> MakeAddPopup(const TArray<FText>& Hints, TFunction<void(const TArray<FString>&)> OnCreate);
