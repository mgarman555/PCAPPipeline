...
void SPCAPActorDatabasePanel::Construct(const FArguments& InArgs)
{
        ...
        [
            ...
            [
                ...
                [
                    ...
                    [
                        SNew(STextBlock)
                        .Text(LOCTEXT("EmptyActors", "No actors yet — type an ID in the box above to create one."))
                        .ColorAndOpacity(FSlateColor(ColText2))
                        .Visibility_Lambda([this]() { return FilteredActors.Num() == 0 ? EVisibility::Visible : EVisibility::Collapsed; })
                    ]
                ]
            ]
        ]
    ...
    ReloadActors();
    RefreshStreaming();
    RegisterActiveTimer(3.0f, FWidgetActiveTimerDelegate::CreateSP(this, &SPCAPActorDatabasePanel::TickStreaming));
}
...
// ── Streaming-now strip: capture untracked live Shogun subjects as actors ───

void SPCAPActorDatabasePanel::RefreshStreaming()
{
    ...
    for (const FString& Name : Untracked)
    {
        ...
        [
            ...
            .ToolTipText(FText::FromString(FString::Printf(TEXT("Add '%s' as an actor, bound to the live body subject"), *Name)))
            .OnClicked_Lambda([this, Name]()
            {
                if (UActorRosterEntry* E = CreateActorAsset(Name))
                {
                    E->DefaultBodyStream.LiveLinkSubjectName = FName(*Name);
                    SaveActorAsset(E);
                    ReloadActors();
                    RefreshStreaming();
                    if (TileView.IsValid()) TileView->SetSelection(TWeakObjectPtr<UActorRosterEntry>(E));
                }
                return FReply::Handled();
            })
            [
                ...
                [ SNew(STextBlock).Text(LOCTEXT("AddActor", "+ actor")).ColorAndOpacity(FSlateColor(ColGreen)) ]
            ]
        ];
    }
...
EActiveTimerReturnType SPCAPActorDatabasePanel::TickStreaming(double, float)
{
    RefreshStreaming();
